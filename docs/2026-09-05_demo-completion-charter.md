...
|---|---|---|
| M1 — finish E5 | t6_switch and t4_while public proofs and production results; assignment protocol and whole-term membership; range review from `8eeaf92` through the completed E5 candidate, including supply normalization. | t5, t6 and t4 execution complete on the current library fragment; t4 now has a decreasing loop invariant, exit/return proof and production result ([record](../cerberus-heaplang/docs/2026-09-06_e5-t4-loop.md)). Full E5 range review and the partial-face API/coverage disposition remain open. |
