...
| provider-native correspondence, strict failures, byte-string parity, broad accepted-domain/completion claims remain open (their master plan steps 2–3, 5); consumer whole-file/library adequacy now covers four supported programs at option (b); the proved frontend connection remains open (our A7) | V1-1 supplies our four-program option-(b) route; §3 R-4 retains the general frontend questions |
...
| R-4 | **the pipeline's whole file as the statement's referent (A7)**, and WHICH pipeline | criterion 4 of the definition of done; master plan V1-1 and G4.3 | SETTLED FOR t1/t5/t6/t4 under the accepted whole-file charters: the pinned Lean frontend consuming OCaml Cabs, then linking and converting the full file; the theorem runs its quoted data with captured comparators under three checks. This is the option-(b) executable boundary. GENERAL PROVIDER GUARANTEES remain OPEN; no equality with OCaml's printed Core is assumed. | The local in-memory loader supplies the bounded one-TU/no-libc route; no new provider API is needed for this four-program cohort. The broader questions remain: structural correspondence between frontends, a stable post-link structured dump (R-7), and option (a), the elaborator in the theorem. Record: `docs/2026-09-08_whole-file-corpus-implementation.md`. |
...
for V1-1a/b by the bounded frontend choice above; their general guarantees
...
| F-5 | **integer semantics with the impl map as a parameter**: `conv_int`/wrap/impl-defined arms, `impl0` bound to the pinned gcc map (R-6's divergence lives here). The out-of-range fallback in `EmittedStdCore.convElse2` is a pure `PEcall (Impl …)`, distinct from the manifest's `Eproc _ (Impl _) _` procedure row. Representable conversions in all four programs take the successful range branch, so retaining the impl map requires no new implementation-procedure rule for this cohort. | the layer's int types (B15 byte images, B21 allocator contracts); future out-of-range conversion rules | the complete-file t1/t5/t6/t4 certificates retain the linked gcc map; all four old wrappers still have `impl0 = ∅` (A7). The captured-library proofs require explicit representability premises. | R-4 settles the bounded linked-file referent; general pure implementation-call support and stability of the parameterized impl map remain layer work, separately from implementation-procedure calls |
