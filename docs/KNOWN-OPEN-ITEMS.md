...
condition proof and both addition RHSs, including the two-read race check;
its assignments and total loop are next
(`cerberus-heaplang/docs/2026-09-06_e5-t4-addition.md`). E5 acceptance
...
| C19 | E5 continuation debts: t4 and the full E5 range audit remain. t6 now certifies the emitted switch and its five registered labels (e5-t6-notes.md). The common t4/t5/t6 load derivation is now factored into Examples/EmittedInt; t4's registered continuations, short-circuit condition and both addition RHSs are proved (e5-t4-notes.md; 2026-09-06_e5-t4-condition.md; 2026-09-06_e5-t4-addition.md). The exact read-footprint rule now supports its two-load race check; its partial twin joins the undemonstrated-face disposition item. Assignment scaffolding and relocation of generic helpers out of concrete exhibits remain cleanup obligations. The t5 checkpoint supplies the slice-2 export pins, five corrected manifest rows, four engine-round coverage witnesses, the API fuel-bound correction and the slice-2 census. Partial E5 rule faces remain unconsumed by a corpus derivation (reported explicitly); seeded supply normalization still needs range review. ARCHITECTURE citation checking repaired 44 clear references but leaves loose/range/usage references for review. | `cerberus-heaplang/docs/2026-09-05_e5-resume.md`; `e5-notes.md` §S2.7/§S2.9 | Complete E5 and audit 8eeaf92..HEAD; refresh the dependency/fuel scout before E6. |
