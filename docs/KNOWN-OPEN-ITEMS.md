...
| C19 | E5 execution milestones are complete on the current library-fragment file: t5 conditional, t6 switch and t4 while have whole-term membership, public total proofs and shipped-driver results. t4 closes its decreasing invariant, exit, both kills and return at budget 915 (2026-09-06_e5-t4-loop.md). The common load/store proof protocol and integer memory-value construction are factored into Examples/EmittedInt; remaining generic-helper relocation and broader exemplar cleanup stay queued. The full E5 range review remains open, including seeded supply normalization and the seven explicitly undemonstrated partial rule faces (useful clients or a reviewed API/coverage disposition required by the charter). The t5 checkpoint supplies the slice-2 pins, five corrected manifest rows, four engine-round witnesses and API fuel-bound correction. ARCHITECTURE citation checking leaves a manual-review queue. The full-file/library connection is still A7. | `cerberus-heaplang/docs/2026-09-05_e5-resume.md`; `2026-09-06_e5-t4-loop.md`; `e5-notes.md` §S2.7/§S2.9 | Audit 8eeaf92..HEAD; refresh the dependency/fuel scout before E6. |
