...
State: candidate `hygiene-h1` head (2026-09-04, after the calls arc, the
fuel-lane restatement F1, the external-audit response AR5, the H1
hygiene/coverage slices and the ARCHITECTURE rewrite). Maintained by the orchestrator; every entry
...
| C14 | The ten `stepDischarge_*` lemmas (Soundness.lean) are consumerless since H1a deleted `outcomesU_of_step` (0 proof-term consumers each, measured); DriverCollapse documents them as its twins. | H1 record, deviation 2 | Delete or re-home; hygiene. |
