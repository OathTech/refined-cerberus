...
| `f(args)` at a WRONG arity | — | (the engine's `failwithI` — a PANIC, not a kill) | `ShippedRefusal.panic` (the engine's own `failwithI` in `call_function`, core_eval.lem:120–163) — [AGENT] classified as the existing PANIC face; no exhibit reaches it |
