# ARCHITECTURE.md — the history archive (2026-09-04)

WHAT THIS IS. The complete text of `cerberus-heaplang/ARCHITECTURE.md`
as it stood at commit `b8413a5` (branch `hygiene-h1`, = main `e242342`
plus the H1a/H1b hygiene slices), copied VERBATIM below, section by
section, before the 2026-09-04 rewrite that answered the fresh full
review `2026-09-04_architecture-review-3.md` (verdict B−, FAIL; its
S-1…S-4 findings: the document was a changelog, not a description).
The rewritten ARCHITECTURE.md describes the system as it IS and points
here for everything that was narrative: arc chronicles, "was … until
…" constructions, superseded designs, slice ids used as vocabulary
(K<n>, C<n>, F1, AR5, P3.5), worker/auditor quotations and dates used
as narrative. Nothing is lost: every sentence of the superseded
document is below.

WHERE THE HISTORY LIVES AUTHORITATIVELY. This archive is a convenience
copy; the records that own each arc are: fragment closure
`2026-09-02_fragment-closure-notes.md`; the kill/free arc
`2026-09-03_kill-free-arc-record.md` (+ `2026-09-03_k{0,1,2,2.5,3,4,5,5.1}-notes.md`
and their audits); the calls arc `2026-09-03_c{1,2,3,4}-notes.md` (+
`2026-09-03_c{1,2}-audit.md`, `2026-09-03_audit-c3-range.md`,
`2026-09-03_audit-c4-range.md`); the re-pin to the fuel arc
`2026-09-03_repin-fuel-notes.md` (scout `../../docs/2026-09-03_repin-scout.md`;
the request `../../docs/2026-09-02_request-cerberus-lean-fuel-exhaustion-outcome.md`);
the fuel-lane restatement `2026-09-03_f1-notes.md` (+ `2026-09-03_audit-f1-range.md`);
the external Reynolds/O'Hearn audit
`../../docs/2026-09-04_reynolds-ohearn-separation-logic-audit.md` and its
response slices `2026-09-04_ar5-readout-notes.md`,
`2026-09-04_ar5-manifest-notes.md`, range audit `2026-09-04_audit-ar5-range.md`;
the hygiene slices `2026-09-04_h1-notes.md`; the rulings register
`../../docs/DECISIONS.md`; the open-items register
`../../docs/KNOWN-OPEN-ITEMS.md`.

WHAT THE REWRITE CARRIED FORWARD (restated in the present tense, with
fresh `file.lean:line` cites verified at `b8413a5`) versus what it moved
here, per section of the superseded text:

| Superseded section | Carried forward into the rewrite | Moved here (history only) |
|---|---|---|
| §1 The semantic authority | the engine as the only semantics; the shipped composite; the trust-as-policy sentence | — |
| §2 The mirror, and its one-directional certification | `Ctl`/`Config`/`Step`; `engine_step_matchU`'s statement; `CerberusRound`; refusals and the residual; `BareHead`/`PePure`; the two bridges | "calls arc C1 made them live, C2 added …"; "the trust rule of 2026-09-02; the 2026-09-03 standards-audit response"; "since the fuel-lane restatement (2026-09-03)"; "since calls arc C4"; the unattributed auditor quotation |
| §3 The two judgments | the atomic specifications; `wps`/`wpt` and their clauses; frame, loop, procedure and call rules; the collapses | the slice tags "K2", "K3", "C3"; the false "metatheorems no export consumes" |
| §4 Adequacy | both lanes over `drive_nonmemory_steps_aux2_lemFuel`; `DriverSafeCtl`/`DriverDoneCtl`; the fuel arms; `wpt_driver_cps` | "the fuel-lane restatement, 2026-09-03"; "calls arc C4"; "the seven earlier production statements' route … the C4 record, §9" |
| §5 The projection | `project_triple_pure`, `MemTriple`, the `*_consequence` lemmas, the client boundary | "measured 2026-09-04 (the Reynolds/O'Hearn audit's Finding 2 …)" |
| §6 The two trust claims, and the two lanes | the trio; the pinned tree's admissions (present tense); the nine closed statements as a table with every input-dependent premise; the closed partial form; the two qualifications | the `Cmm_op.lean` `sorry` history; "since calls arc C4"; the 40-line root-of-trust sentence; "the DECISIONS register's 'nine production statements'"; the `driveU` paragraph and the fuel-arc request narrative |
| §7 Open items, and the acceptance goals | the three goals with status and [USER] provenance; the not-covered list | the three CLOSED arc chronicles (region access K5, kill/free K0–K5.1, calls C1–C4); the two-label and mutual-recursion closure narratives; the instrument paragraphs' slice history ("formerly counted", "the two the manifest slice carried while `ar5-readout` was in flight", "the six stale seeds of the F1 renaming") |

---

# The superseded text, verbatim (`git show b8413a5:cerberus-heaplang/ARCHITECTURE.md`)
