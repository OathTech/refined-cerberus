...
| The small axioms, proved once | `AtomicStep` (the mask-generic one-step-to-value specification) and the five atomic specifications `store_atomic`, `load_atomic`, `storeAt_atomic`, `loadAt_atomic`, `create_atomic`; the lifting lemmas `wp_of_atomic` (Rules), `wps_of_atomic` (Wps), `wpt_of_atomic` (Wpt) — every memory rule of every judgment is a corollary; `deliveryCost`, `createExpr` (Rules) |
...
| memM lemmas and byte-map algebra | `storeM_success`, `loadM_success`, `loadM_at`, `storeM_at`, `writeBytesTo_*`, `readBytesFrom_*`, `byteAt_*`, `MetaByteOf`, `spikeCells_alloc`, `intToBytes_*`, `bytesToInt_*` (Heap, Adequacy) | Engine-memory facts consumed by the rule proofs and the launch |
