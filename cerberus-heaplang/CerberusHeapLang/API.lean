...
`drive_nonmemory_steps_aux2_lemFuel` at every loop counter and every
ambient budget of at least two (`[LemFuel]`, `hfuel : 2 ≤ LemFuel.fuel`;
docs/FUEL.md), from any driver state holding the configuration
(`DriverSafeCtl`, Adequacy.lean), with the out-of-fuel arm the
kernel-transparent kill `CerbND.fuelExhaustedKill`; the closed forms over
the shipped pipeline `runND ∘ drive ∘ initial_driver_state` — total
(`exhibitA_prod`, `*_certified_production`, `prod_run_eqJ`; their
shipped-constant instances `*_shipped`, Shipped.lean) and partial
(`prod_run_safe_procs`, `fib_rec_certified`, at every ambient `[LemFuel]`
of the shipped `drive`) — live in the production layer this module does
not import. No
...
| Family | Names (module) |
|---|---|
| Program vocabulary | `CoreExpr`, `EnvStack`, `SpikeVal` (`.pure`/`.annot`, `mergeInto`), `ofVal`, `MachineCtx` (`.tagDefs`, `.extern`, `.labels`, `.SeqWF`), `spikeCtx`, `spikeEnv`, `symPat`, `storeOpRedex` (Step); `storeExpr`, `loadExpr`, `sseqExpr`, `addrOf`, `loadedVal` (Rules); `createExpr` (Rules); `Frag` — fragment membership, the adequacy premise (Fragment — SYNTACTIC, R2 2026-09-07; its companion measure `evalDepth` is the depth hypothesis's; the plain-symbol binder `Frag.sseq_sym` admits any fragment head since E1 (both LETS betas are mirrored); a procedure call head since calls arc C4: `lets x = f(args) in …`) |
...
| Side-condition vocabulary (appears in rule statements) | `StorableAt`, `StorableView` (its four-field face for the typed-subrange stores; `StorableAt.toView`), `decIndep` with `decIndep_array_integer` and the layout equation `sizeofCtype_array_integer`, `atomicTy`, `loadTrapV`, `cellLoadTrap`, `undefByte`, `spliceBytes` with `spliceBytes_length`/`_getElem?`/`_slice_below`/`_slice_self`/`_slice_above` (Heap) |
...
| The engine transition and its certification | `Step` and every `Step.*` lemma, `Decomp`, `jumpRedex?`, `toVal`, `primStep`, the `Language` instance's facts (Lang); all of Soundness including the proof device `FragFuel` (the fuel-relative twin of `Frag`, bridged by `Frag.toFuel`; the `*_fuel` device forms of the exports are unpinned); all of Round (`CerberusRound`, `cerberusRound_classify`, `step_iff_cerberusRound`); the driver collapse's round lemmas (`loop_step_frag`, `loop_step_frag'`, `loop_step_frag_same`, `loop_step_frag_same'`, the `loop_step_*` iteration lemmas, `driver2_done`/`driver2_killed`, `finalize_done`, `runND_active`/`runND_killed`; DriverCollapse, imported through Adequacy since the fuel-lane restatement) | They certify the rules against the engine. A client reasoning through them bypasses the logic |
...
is reusable when another file supplies the same library data.
`EmittedIntSupport` supplies annotated whole-cell loads at arbitrary
fraction, evaluated-operand signed-int assignments, symbol evaluation
under extern comparison equality, and the unsequenced rule for a literal
