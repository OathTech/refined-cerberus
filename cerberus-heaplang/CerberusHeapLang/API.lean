...
| Adequacy (total) — PROVISIONAL, over `driveU` (header) | `wpt_engine_boundU`, `wpt_engine_boundU_alloc`, `readoutPost`, `DriveDoneAt` (TotalAdequacy) — at the EMPTY procedure table. The root-of-trust total statements over the shipped driver (`exhibitA_prod`, `*_certified_production`, `prod_run_eqJ`, and through calls `prod_run_eqJ_procs` with the CPS driver lane `wpt_driver_cps`/`wpt_driver_done_procs`, calls arc C4) are in the production layer, not imported here |
...
`DriverCollapse`, `ProdLoop` (incl. the lane through calls:
`DriverDoneCtl`, `LabeledProcs`, `wpt_driver_cps`,
`wpt_driver_done_procs`), `ProdEntry` (`prodFile`, `prodFileWith`,
`prodCtl`, `prodCtx`, `prod_run_eqJ`, `prod_run_eqJ_procs`) — the
production-export layer: itself a client of adequacy, imported directly
by the production exhibits — every `*Exhibit`, `Examples.*`, and `Audit`.
-/
