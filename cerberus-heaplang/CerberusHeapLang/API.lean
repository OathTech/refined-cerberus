...
| The pure memory view (the boring post's vocabulary) | `CellCoh` (one cell: live, writable, in bounds, exactly those bytes, decode-inert), `Coh` = `Sat` (a footprint of pairwise-disjoint such cells), `SpikeCell`, `CellMap`, `readBytesFrom`, `cellsDisjoint` (Heap, Adequacy) — what the `*_consequence` lemmas deliver about the final memory and what `MemTriple`'s launch premise demands of the initial one |
...
| The discharge devices of Round.lean's classification | the hand-written discharge `dischargeStep`/`outcomesU` and the lemmas stated over them — `stepDischarge_*`, `outcomesU_of_step`, `outcomesU_done`, `outcomesU_remove_annot` (Soundness) | Proof devices (the trust rule of 2026-09-02): their statements' referent is a package definition, not the engine, so they are not exports — unpinned, bounded by the exhaustive sweep; since the fuel-lane restatement no adequacy lane consumes them (both lanes iterate the shipped round `loop_step_frag`) |
