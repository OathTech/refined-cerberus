...
theorem tree_rotate_wps
    ...
    isTree (GF := GF) px (.node idx vx (.node idy vy ta tb) tc) ⊢
      wps spikeCtx none Ls emptyProcSpec
  ...
  rw [show cellPtr idx (aX + 8) = cellPtr idx (aX + ((8 : Nat) : Int))
    from rfl]
  iapply wps_load_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpec) loc ann idx aX 8 mo (.own 1) bsx _
  ...
  rw [show cellPtr idy (aY + 16) = cellPtr idy (aY + ((16 : Nat) : Int))
    from rfl]
  iapply wps_load_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpec) loc ann idy aY 16 mo (.own 1) bsy _
    (by rw [treeTy_size]; omega)
  ...
  rw [show cellPtr idx (aX + 8) = cellPtr idx (aX + ((8 : Nat) : Int))
    from rfl]
  iapply wps_store_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpec) loc ann idx aX 8 (ptrVal qb) mo bsx _
  ...
  rw [show cellPtr idy (aY + 16) = cellPtr idy (aY + ((16 : Nat) : Int))
    from rfl]
  iapply wps_store_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpec) loc ann idy aY 16 (ptrVal (cellPtr idx aX))
    mo bsy _
...
theorem tree_rotate_wpt
    ...
    (px : CerbMem.PointerValue) :
    isTree (GF := GF) px (.node idx vx (.node idy vy ta tb) tc) ⊢
      wpt spikeCtx none Ls emptyProcSpecT 19
        (trPost (.node idy vy ta (.node idx vx tb tc)))
        (trProg loc ann mo xbty ybty bbty ubty px)
        [fmapEmpty] := by
  iintro HX
  ...
  rw [show cellPtr idx (aX + 8) = cellPtr idx (aX + ((8 : Nat) : Int))
    from rfl]
  iapply wpt_load_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpecT) loc ann idx aX 8 mo (.own 1) bsx _
  ...
  rw [show cellPtr idy (aY + 16) = cellPtr idy (aY + ((16 : Nat) : Int))
    from rfl]
  iapply wpt_load_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpecT) loc ann idy aY 16 mo (.own 1) bsy _
    (by omega) (by rw [treeTy_size]; omega)
    (fun lum fpm => hRy lum fpm _)
  isplitl [HptY]
  · iexact HptY
  ...
  rw [show cellPtr idx (aX + 8) = cellPtr idx (aX + ((8 : Nat) : Int))
    from rfl]
  iapply wpt_store_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpecT) loc ann idx aX 8 (ptrVal qb) mo bsx _
    (by omega)
    (tree_ptr_encodes qb) (by rw [treeTy_size]; omega) kBlen
    (tree_ptr_compat qb) kBfpm kBbytes
  isplitl [HptX]
  ...
  rw [show cellPtr idy (aY + 16) = cellPtr idy (aY + ((16 : Nat) : Int))
    from rfl]
  iapply wpt_store_tree_field (M := spikeCtx) (p := none) (Θ := emptyProcSpecT) loc ann idy aY 16 (ptrVal (cellPtr idx aX))
    mo bsy _
    (by omega)
    (tree_ptr_encodes (cellPtr idx aX)) (by rw [treeTy_size]; omega) kXlen
    (tree_ptr_compat (cellPtr idx aX)) kXfpm kXbytes
