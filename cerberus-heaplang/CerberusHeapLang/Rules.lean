...
axioms, sequencing, frame, consequence.
...
- The first derived exhibits ({x ↦ - ∗ y ↦ a} store(x,7) {x ↦ 7 ∗ y ↦ a}
  by FRAME on the store small axiom; the two-store `exhibitC_triple`)
  and the anti-frame negative transcript live in
  `Examples/Layout.lean` with the example constants they use (alloc
  arc P5, R-07): this module contains rules and their supporting
  lemmas only.
