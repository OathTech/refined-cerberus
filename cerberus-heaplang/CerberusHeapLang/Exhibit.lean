...
(a) store-then-load: on a concrete state seeded through the engine's
    ...
    at every loop iteration counter, with ambient fuel at least two,
    `DriverSafeCtl`).
(b) the frame exhibit, discharged end-to-end:
    ...
    lands as an engine fact: store(x,7) exhausts or delivers, and
    on delivery the real MemState holds 7's bytes at x and y's bytes
...
The complete fixture state and its pointers are fuel-independent.
Examples.SeededMemory proves their agreement with these allocations at
every positive ambient budget, and records the zero-budget failure.
The client establishes Coh for this state below.
-/
import CerberusHeapLang.API
import CerberusHeapLang.Examples.Layout
import CerberusHeapLang.Examples.SeededMemory
...
/- The fuel-independent fixture and its engine-allocation agreement are
   in Examples.SeededMemory. The following proofs establish the client
   preconditions, then reason through the public logic rules. -/
...
theorem provenB [LemFuel] {GF : BundledGFunctors} [SpikeGpreS GF] :
...
theorem spike_blockSpecs [LemFuel] [SpikeGS .hasLC GF] (Ψ : SpikeVal → EnvStack → IProp GF) :
...
theorem provenA [LemFuel] {GF : BundledGFunctors} [SpikeGpreS GF] :
...
/-- EXHIBIT (a) AS A SEMANTIC TRIPLE: over EVERY engine configuration
    ...
    `lets _ = store(x,7) in load(x)` exhausts or delivers, and any
...
theorem exhibitB_semantic [LemFuel] (hfuel : 2 ≤ LemFuel.fuel) {GF : BundledGFunctors} [SpikeGpreS GF] :
...
/-- Exhibit (a) at the seeded engine instance (rest := ∅): from any
    ...
    the shipped loop at every iteration counter exhausts or delivers
    `Specified(7)`, under the stated ambient-fuel bound. -/
theorem exhibitA_engine [LemFuel] (hfuel : 2 ≤ LemFuel.fuel) :
    DriverSafeCtl spikeCtx (spikeThread progA) progA spikeEnv spikeCtl σ₀
      (fun v _ => v = sevenVal) := by
  refine (exhibitA_semantic (hfuel := hfuel) (GF := SpikeGF) ∅
...
theorem exhibitB_engine [LemFuel] (hfuel : 2 ≤ LemFuel.fuel) :
  ...
  have hcohBu : Sat fmapEmpty σ₀ (Iris.Std.PartialMap.union
    ...
    · intro id1 id2 c1 c2 hne h1 h2
      ...
      · exact cells_disjoint
      · exact absurd rfl hne
  refine (exhibitB_semantic (hfuel := hfuel) (GF := SpikeGF) ∅
...
theorem progA_wpt [LemFuel] {GF : BundledGFunctors} [SpikeGS .hasLC GF]
...
theorem provenC [LemFuel] {GF : BundledGFunctors} [SpikeGpreS GF] :
...
/-- EXHIBIT (c) AS A SEMANTIC TRIPLE: over EVERY engine configuration
    ...
    drive of `lets _ = store(x,5) in store(y,6)` exhausts or
    delivers, and any completed run updates BOTH cells (x to 5's bytes,
...
/-- Exhibit (c) at the seeded engine instance (rest := ∅): after the
    ...
    memory. -/
theorem exhibitC_engine [LemFuel] (hfuel : 2 ≤ LemFuel.fuel) :
  ...
  refine (exhibitC_semantic (hfuel := hfuel) (GF := SpikeGF) ∅
