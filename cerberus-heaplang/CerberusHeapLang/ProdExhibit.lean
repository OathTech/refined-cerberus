...
record is in docs/2026-09-01_p2-notes.md), stores 7 through the
bound pointer and loads it back:

  main() = lets p = create(4, int) in
           lets v = 7 in
           lets _ = store(int, p, v) in load(int, p)
...
at derived budget 11) + the GENERIC adequacy/driver collapse
...
/-! ## The frame after the two binds, and its lookups -/

/-- The head frame after `p` and `v` are bound. -/
abbrev prodAFrame (p : CerbMem.PointerValue) (f : Fmap sym value) :
    Fmap sym value :=
  envAdd vASym sevenVal (envAdd pASym (Vobject (OVpointer p)) f)
...
theorem prodAFrame_lookup_v {f : Fmap sym value} (hf : SymFrame f)
    (p : CerbMem.PointerValue) :
    fmapLookupBy symCmpK vASym (prodAFrame p f) = some sevenVal := by
  unfold prodAFrame
...
/-- THE WHOLE PROGRAM AT THE TOTAL JUDGMENT, budget 11 (derived:
    ...
    load-operand eval 1 + load 3): from `allocCap [⟨4, intTy⟩]`
    ALONE — the create through the PUBLIC `wpt_create`, the
    store/load through the generic heap rules at the PROGRAM-BOUND
    pointer. -/
theorem progAProd_wpt [SpikeGS .hasLC GF]
  ...
  · ipureintro
    rfl
  rw [update_env_sym pASym BTy_unit, show (9 : Nat) = 1 + 8 from rfl]
  iapply wpt_seq_sym
  ...
  obtain ⟨dres, dst', heq, hψ, hbl, hout, herr⟩ :=
    prod_run_eqJ sup progAProd hQe (ψA fmapEmpty) 11
      (wpt_driver_done_alloc (GF := SpikeGF)
        ...
        (by rw [show pot progAProd = 5 from rfl,
