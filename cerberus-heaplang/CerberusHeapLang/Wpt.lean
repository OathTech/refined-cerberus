        ...
        | k' + 1 =>
          iprop(∀ (κ : List (Option sym × context)) (ℓ : exec_location)
            ...
            ∀ (r : CoreRt) (σ₂ : Mem) (eₜ : List CoreRt),
              ⌜((⟨e, ρ, ⟨κ, p, ℓ, lc, sp⟩, M⟩ : CoreRt), σ₁) -<([] : List Empty)>-> (r, σ₂, eₜ)⌝
                ={∅,⊤}=∗
...
theorem wpt_frame_labels {Ψ : SpikeVal → EnvStack → IProp GF} (R : IProp GF)
  ...
  | none =>
    ...
    | some q =>
      ...
      iintro %ret %a1 Hpost
      ihave H' := Hcont $$ %ret %a1 Hpost
      iapply IH k₁ (by omega) (apply_ctx ctx (ofValA (.pure a1 [] ret))) ρ
        $$ H' HR
    | none =>
      cases k with
      | zero =>
...
/-- Value rule at ANY static annotation lists (E1). -/
theorem wpt_ofValA {Ψ : SpikeVal → EnvStack → IProp GF} (w : SpikeValA)
    (ρ : EnvStack) {k : Nat} (hk : deliveryCost w.erase ≤ k) :
    Ψ w.erase ρ ⊢ wpt M p Ls Θ k Ψ (ofValA w) ρ := by
  rw [wpt_val_eq k (toVal_ofValA w)]
  iintro H
  isplit
  · ipureintro; exact hk
  · imodintro
...
theorem wpt_det_step {Ψ : SpikeVal → EnvStack → IProp GF} {e : CoreExpr}
    ...
    (hstep : ∀ (κ : List (Option sym × context)) (ℓ : exec_location) (lc : CerbLocation.Loc)
      (sp : RunSup) (σ : Mem),
      Step M (e, ρ, ⟨κ, p, ℓ, lc, sp⟩, σ)
        (e', ρ', (⟨κ, p, ℓ, lc, sp⟩ : Ctl).upd (rootAnnots e), σ))
    (hdet : ∀ (κ : List (Option sym × context)) (ℓ : exec_location) (lc : CerbLocation.Loc)
      (sp : RunSup) (σ : Mem) (out : Config),
      Step M (e, ρ, ⟨κ, p, ℓ, lc, sp⟩, σ) out →
  ...
  isplitr
  · ipureintro
    exact ⟨[], ⟨e', ρ', (⟨κ, p, ℓ, lc, sp⟩ : Ctl).upd (rootAnnots e), M⟩, σ₁, [],
      hstep κ ℓ lc sp σ₁, rfl, rfl⟩
  iintro %r %σ₂ %eₜ %Hstep
  obtain ⟨hs, hlbl, rfl⟩ := Hstep
  have hout := hdet κ ℓ lc sp σ₁ _ hs
...
theorem wpt_if {Ψ : SpikeVal → EnvStack → IProp GF} (a : List annot)
  ...
  · rw [show (bif false then e2 else e3) = e3 from rfl]
    iintro ⟨%hg, H⟩
    iapply wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.if_false hg)
  ...
  · rw [show (bif true then e2 else e3) = e2 from rfl]
    iintro ⟨%hg, H⟩
    iapply wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.if_true hg)
      (fun _ _ _ _ σ out hs => by
...
theorem wpt_save_vals {Ψ : SpikeVal → EnvStack → IProp GF} (a : List annot)
  ...
  wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.save hvals)
    (fun _ _ _ _ σ out hs => by
...
theorem wpt_load_eval {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.load_eval hnv2 hv2)
    (fun _ _ _ _ σ out hs => by
      ...
      simpa [loadExpr, loadOpRedex] using hout)
...
theorem wpt_kill_eval {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.kill_eval hnv hv)
    (fun _ _ _ _ σ out hs => by
      ...
      simpa [killExpr, killOpRedex] using hout)
...
theorem wpt_store_eval {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.store_eval hnv hv2 hv3)
    (fun _ _ _ _ σ out hs => by
      ...
      simpa [storeExpr, storeOpRedex] using hout)
...
theorem wpt_memop_eval {Ψ : SpikeVal → EnvStack → IProp GF} (a : List annot)
  ...
  wpt_det_step rfl rfl rfl (fun _ _ _ _ _ => Step.memop_eval hnv hv1 hv2)
    (fun _ _ _ _ σ out hs => by
...
/-- The pointer-equality memop at value operands: one
    ...
    2 ≤ k). -/
theorem wpt_memop_ptreq {Ψ : SpikeVal → EnvStack → IProp GF} (a : List annot)
    ...
    Ψ (.pure (boolValue b)) ρ ⊢
      wpt M p Ls Θ k Ψ (memopPtrEqVals a (Vobject (OVpointer pv1))
  ...
  exact wpt_det_step rfl rfl rfl
    (fun _ _ _ _ σ => Step.memop_ptreq rfl rfl (hres σ))
    (fun _ _ _ _ σ out hs => by
...
theorem wpt_annot_reindex (aA aB : List annot) (dsA dsB : List dyn_annotation)
    (c : CoreExpr) (ρ : EnvStack) {k : Nat}
    {Ψ₁ Ψ₂ : SpikeVal → EnvStack → IProp GF}
    (hΦ : ∀ w ρ', Ψ₁ (SpikeVal.merge dsA w) ρ' = Ψ₂ (SpikeVal.merge dsB w) ρ') :
    wpt M p Ls Θ k Ψ₁ (Expr aA (Eannot dsA c)) ρ ⊢
      wpt M p Ls Θ k Ψ₂ (Expr aB (Eannot dsB c)) ρ := by
  induction k using Nat.strongRecOn generalizing aA aB dsA dsB c ρ Ψ₁ Ψ₂ with
  | ind k IH =>
  ...
  · -- non-value on both sides
    ...
    | some q =>
      ...
      iintro %ret %a1 Hpost
      ihave H' := Hcont $$ %ret %a1 Hpost
      iapply IH k' (by omega) aA aB dsA dsB (apply_ctx c₀ (ofValA (.pure a1 [] ret))) ρ hΦ $$ H'
    | none =>
        ...
        · iintro %e₂ %σ₂ %eₜ %HstepB
          ...
          · obtain ⟨e₂e, e₂ρ, e₂ctl, e₂M⟩ := e₂
            ...
            imod H $$ %(⟨Expr aA (Eannot dsA c'), e₂ρ, e₂ctl, M⟩ : CoreRt) %_ %([])
              %⟨Step.annot_ctx hnj hnc hnv hg hs, rfl, rfl⟩ with ⟨$, H⟩
            imodintro
            iapply IH m (Nat.lt_succ_self m) aA aB dsA dsB c' e₂ρ hΦ $$ H
          · obtain ⟨e₂e, e₂ρ, e₂ctl, e₂M⟩ := e₂
            simp only at hlbl
            obtain rfl : M = e₂M := hlbl.symm
            ...
            imodintro
            iapply IH m (Nat.lt_succ_self m) (aA ++ a2) (aB ++ a2) (dsA ++ ds2) (dsB ++ ds2)
              c'' ρ
              (fun w ρ' => by
...
/-- `wpt` commutes with the run-time dyn-annotation wrapper: one
    ...
    or a value-forming wrap costs none — the unit is slack there). E1: at
    ANY static annotations `a` on the wrapper node. -/
theorem wpt_annot (a : List annot) (ds : List dyn_annotation) (e : CoreExpr) (ρ : EnvStack)
    {k : Nat} {Ψ : SpikeVal → EnvStack → IProp GF} :
    wpt M p Ls Θ k (fun w ρ' => Ψ (SpikeVal.merge ds w) ρ') e ρ ⊢
      wpt M p Ls Θ (k + 1) Ψ (Expr a (Eannot ds e)) ρ := by
  induction k using Nat.strongRecOn generalizing a ds e ρ with
  | ind k IH =>
  ...
  | some w =>
    ...
    | pure a1 b1 v =>
      ...
      rw [wpt_val_eq k (toVal_ofValA (.pure a1 b1 v)),
        wpt_val_eq (k + 1)
      ...
      · imod H with H
        imodintro
        rw [show (SpikeVal.annot ds v) = SpikeVal.merge ds (SpikeValA.pure a1 b1 v).erase
          from rfl]
        iexact H
    | annot a1 a2 b1 ds2 v =>
      -- double annot: one deterministic ANNOTS-merge step to a value
      rw [wpt_val_eq k (toVal_ofValA (.annot a1 a2 b1 ds2 v)),
        wpt_step_eq k
      ...
      · obtain ⟨rfl, rfl, rfl⟩ : a1 = a2' ∧ ds2 = ds2' ∧
        ...
        · rw [show Expr (a ++ a1) (Eannot (ds ++ ds2) (Expr a2 (Epure (Pexpr b1 () (PEval v))))) =
          ...
          iapply wpt_ofValA (.annot (a ++ a1) a2 b1 (ds ++ ds2) v) ρ
            (by simp only [SpikeValA.erase_annot, deliveryCost_annot]; omega)
    ...
    · -- plain body: jump-clause transfer, or lockstep reduction
      ...
      | some q =>
        ...
        rw [wpt_call_eq hv hjr hcr,
          wpt_call_eq hwrap ((jumpRedex?_annot_of_not_root a ds hr').trans hjr)
            (show callRedex? (Expr a (Eannot ds e)) =
                some (Cannot a ds ctx, f, pes) by
              rw [callRedex?_annot_of_not_root a ds hr', hcr]; rfl)]
        simp only [apply_ctx_annot]
        iintro H
        imod H with ⟨%params, %body, %vs, %m, %k', %hb, %h1, %h2, %h3, Hpre, Hcont⟩
        imodintro
        ...
        iintro %ret %a1 Hpost
        ihave H' := Hcont $$ %ret %a1 Hpost
        iapply IH k' (by omega) a ds (apply_ctx ctx (ofValA (.pure a1 [] ret))) ρ $$ H'
      | none =>
        cases k with
        | zero =>
        ...
        | succ m =>
          ...
          · obtain ⟨e₂e, e₂ρ, e₂ctl, e₂M⟩ := e₂
            ...
            obtain ⟨he, hρ, hctl, hσ⟩ : e₂e = Expr a (Eannot ds e'') ∧
              ...
              simpa [Prod.mk.injEq] using hout
            subst he hρ hctl hσ
            imod H $$ %(⟨e'', e₂ρ, e₂ctl, M⟩ : CoreRt) %_ %([]) %⟨hs, rfl, rfl⟩
              with ⟨$, H⟩
            imodintro
            iapply IH m (Nat.lt_succ_self m) a ds e'' e₂ρ $$ H
          · exact absurd heq (by
              intro heq
              rw [heq] at hr'
...
/-- E2: THE TOTAL strong-sequencing rule at a flat tuple binder (total analog
    of `wps_seq_tuple`). -/
theorem wpt_seq_tuple {Ψ : SpikeVal → EnvStack → IProp GF}
    (a pa : List annot) (ls : List TupleLeaf)
    (e1 e2 : CoreExpr)
    (ev0 : Fmap sym value) (evs : List (Fmap sym value)) (k1 k2 : Nat) :
    wpt M p Ls Θ k1 (fun w ρ' => iprop(∃ (vs : List value),
        ⌜w = SpikeVal.pure (Vtuple vs)⌝ ∗
        wpt M p Ls Θ k2 Ψ e2 (update_env (tuplePat pa ls) (Vtuple vs) ρ')))
      e1 (ev0 :: evs) ⊢
      wpt M p Ls Θ (k1 + k2) Ψ (Expr a (Esseq (tuplePat pa ls) e1 e2))
        (ev0 :: evs) := by
  induction k1 using Nat.strongRecOn generalizing e1 ev0 evs with
  | ind k1 IH =>
  cases htv : toVal e1 with
  ...
  | none =>
    ...
    | some q =>
      obtain ⟨ctx, f, pes⟩ := q
      rw [wpt_call_eq htv hjr hcr,
        wpt_call_eq (toVal_sseq_node a (tuplePat pa ls) e1 e2)
          (by rw [jumpRedex?_sseq]; exact hjr)
          (show callRedex? (Expr a (Esseq (tuplePat pa ls) e1 e2)) =
              some (Csseq a (tuplePat pa ls) ctx e2, f, pes) by
            rw [callRedex?_sseq, hcr]; rfl)]
      simp only [apply_ctx_sseq]
      iintro H
      imod H with ⟨%params, %body, %vs, %m, %k', %hb, %h1, %h2, %h3, Hpre, Hcont⟩
    ...
    | none =>
      ...
      | succ m =>
        ...
        · ipureintro
          obtain ⟨obs0, r', σ', eₜ', hps⟩ := hred
          obtain ⟨hs', hlbl', hnil'⟩ := hps
          exact ⟨[], ⟨Expr a (Esseq (tuplePat pa ls)
              r'.e e2), r'.ρ, r'.ctl, M⟩, σ', [],
            ⟨Step.sseq_ctx hjr hcr htv hs', rfl, rfl⟩⟩
        iintro %r %σ₂ %eₜ %Hstep
        obtain ⟨hs, hlbl, rfl⟩ := Hstep
...
/-- E2: THE TOTAL weak-sequencing rule at the plain-symbol binder (total
    analog of `wps_wseq_sym`). -/
theorem wpt_wseq_sym {Ψ : SpikeVal → EnvStack → IProp GF}
    (a pa : List annot) (x : sym) (bty : core_base_type)
    (e1 e2 : CoreExpr)
    (ev0 : Fmap sym value) (evs : List (Fmap sym value)) (k1 k2 : Nat) :
    wpt M p Ls Θ k1 (fun w ρ' => iprop(∃ (v : value),
        ...
        wpt M p Ls Θ k2 Ψ e2 (update_env (symPat pa x bty) v ρ')))
      e1 (ev0 :: evs) ⊢
      wpt M p Ls Θ (k1 + k2) Ψ (Expr a (Ewseq (symPat pa x bty) e1 e2))
        (ev0 :: evs) := by
  induction k1 using Nat.strongRecOn generalizing e1 ev0 evs with
  | ind k1 IH =>
  cases htv : toVal e1 with
  ...
  | none =>
    ...
    | some q =>
      obtain ⟨ctx, f, pes⟩ := q
      rw [wpt_call_eq htv hjr hcr,
        wpt_call_eq (toVal_wseq_node a (symPat pa x bty) e1 e2)
          (by rw [jumpRedex?_wseq]; exact hjr)
          (show callRedex? (Expr a (Ewseq (symPat pa x bty) e1 e2)) =
              some (Cwseq a (symPat pa x bty) ctx e2, f, pes) by
            rw [callRedex?_wseq, hcr]; rfl)]
      simp only [apply_ctx_wseq]
      iintro H
      imod H with ⟨%params, %body, %vs, %m, %k', %hb, %h1, %h2, %h3, Hpre, Hcont⟩
    ...
    | none =>
      ...
      | succ m =>
        ...
        · ipureintro
          obtain ⟨obs0, r', σ', eₜ', hps⟩ := hred
          obtain ⟨hs', hlbl', hnil'⟩ := hps
          exact ⟨[], ⟨Expr a (Ewseq (symPat pa x bty)
              r'.e e2), r'.ρ, r'.ctl, M⟩, σ', [],
            ⟨Step.wseq_ctx hjr hcr htv hs', rfl, rfl⟩⟩
        iintro %r %σ₂ %eₜ %Hstep
        obtain ⟨hs, hlbl, rfl⟩ := Hstep
...
/-- E2: THE TOTAL weak-sequencing rule at a flat tuple binder (total analog of
    `wps_wseq_tuple`). -/
theorem wpt_wseq_tuple {Ψ : SpikeVal → EnvStack → IProp GF}
    (a pa : List annot) (ls : List TupleLeaf)
    (e1 e2 : CoreExpr)
    (ev0 : Fmap sym value) (evs : List (Fmap sym value)) (k1 k2 : Nat) :
    wpt M p Ls Θ k1 (fun w ρ' => iprop(∃ (vs : List value),
...
theorem wpt_load_at {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(pointsToView M.tagDefs (GF := GF) id a aty off dqm dqb vty bs ∗
      ...
      wpt M p Ls Θ k Ψ (loadExpr an loc ann vty (cellPtr id (a + (off : Int))) mo)
        ρ := by
  iintro ⟨Hv, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => loadAt_atomic an loc ann id a aty off vty mo dqm dqb bs ρ hdec htrap)
    rfl rfl rfl hk
  isplitl [Hv]
  · iexact Hv
  · iintro %w ⟨%fp, %hw, Hv'⟩
...
theorem wpt_store_at {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(pointsToView M.tagDefs (GF := GF) id a aty off dqm (.own 1) vty bs ∗
      ...
      wpt M p Ls Θ k Ψ (storeExpr an loc ann vty (cellPtr id (a + (off : Int))) cv mo)
        ρ := by
  iintro ⟨Hv, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => storeAt_atomic an loc ann id a aty off vty cv mo dqm bs ρ hmv hst)
    rfl rfl rfl hk
  isplitl [Hv]
  · iexact Hv
  · iintro %w ⟨%fp, %hw, Hv'⟩
...
theorem wpt_load_cell_at {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  · rw [show a + (off : Int) + ((CerbMem.sizeofCtype M.tagDefs vty : Nat) : Int) =
      a + ((bs.take off).length : Int) +
        ...
        rw [htk, hmidlen]]
    iexact Hsuf0
  iapply wpt_load_at an loc ann id a aty off vty mo dq dq
...
theorem wpt_store_cell_at {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  · rw [show a + (off : Int) + ((CerbMem.sizeofCtype M.tagDefs vty : Nat) : Int) =
      a + ((bs.take off).length : Int) +
        ...
        rw [htk, hmidlen]]
    iexact Hsuf0
  iapply wpt_store_at an loc ann id a aty off vty cv mo (.own 1)
...
theorem wpt_load_region_at {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(typedRegionView M.tagDefs (GF := GF) id a n off dqm dqb vty bs ∗
      ...
      wpt M p Ls Θ k Ψ (loadExpr an loc ann vty (cellPtr id (a + (off : Int))) mo)
        ρ := by
  iintro ⟨Hv, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => regionLoadAt_atomic an loc ann id a n off vty mo dqm dqb bs ρ hdec htrap)
    rfl rfl rfl hk
  isplitl [Hv]
  · iexact Hv
  · iintro %w ⟨%fp, %hw, Hv'⟩
...
theorem wpt_store_region_at {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(typedRegionView M.tagDefs (GF := GF) id a n off dqm (.own 1) vty bs ∗
      ...
      wpt M p Ls Θ k Ψ (storeExpr an loc ann vty (cellPtr id (a + (off : Int))) cv mo)
        ρ := by
  iintro ⟨Hv, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => regionStoreAt_atomic an loc ann id a n off vty cv mo dqm bs ρ hmv hst)
    rfl rfl rfl hk
  isplitl [Hv]
  · iexact Hv
  · iintro %w ⟨%fp, %hw, Hv'⟩
...
theorem wpt_load_regionOwn_at {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  have hsplit : bs = bs.take off ++
      ((bs.drop off).take (CerbMem.sizeofCtype M.tagDefs vty) ++
        (bs.drop off).drop (CerbMem.sizeofCtype M.tagDefs vty)) := by
    rw [List.take_append_drop, List.take_append_drop]
  iapply wpt_load_region_at an loc ann id a n off vty mo dq dq _ ρ hk hdec htrap
  isplitl [Hv]
  · iexact Hv
  iintro %fp Hv
  iapply HΨ
...
theorem wpt_store_regionOwn_at {Ψ : SpikeVal → EnvStack → IProp GF}
  ...
  have hlen' : (bs.take off ++ ((CerbMem.memValueToBytes M.tagDefs [] mv).2 ++
    ...
    omega
  iapply wpt_store_region_at an loc ann id a n off vty cv mo (.own 1) _ ρ hk hmv hst
  isplitl [Hv]
  · iexact Hv
  iintro %fp Hv
  iapply HΨ
...
theorem wpt_store {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(pointsToCell M.tagDefs (GF := GF) pv (.own 1) ty bs ∗
      (∀ fp, pointsToCell M.tagDefs pv (.own 1) ty (CerbMem.memValueToBytes M.tagDefs [] mv).2 -∗
        Ψ (SpikeVal.annot [DA_pos [] fp] Vunit) ρ)) ⊢
      wpt M p Ls Θ k Ψ (storeExpr a loc ann ty pv cv mo) ρ := by
  iintro ⟨Hpt, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => store_atomic a loc ann ty pv cv mo mv bs ρ hmv hst) rfl rfl rfl hk
  isplitl [Hpt]
  · iexact Hpt
  · iintro %w ⟨%fp, %hw, Hpt'⟩
    subst hw
...
theorem wpt_kill {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(pointsToCell M.tagDefs (GF := GF) pv (.own 1) ty bs ∗
      ...
      wpt M p Ls Θ k Ψ (killExpr a loc ann kind pv) ρ := by
  iintro ⟨Hpt, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => kill_atomic a loc ann kind pv ty bs ρ hstatic) rfl rfl rfl hk
  isplitl [Hpt]
  · iexact Hpt
  · iintro %w ⟨%hw, Hd⟩
    subst hw
...
theorem wpt_kill_emp {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(pointsToCell M.tagDefs (GF := GF) pv (.own 1) ty bs ∗ Ψ (SpikeVal.pure Vunit) ρ) ⊢
      wpt M p Ls Θ k Ψ (killExpr a loc ann kind pv) ρ := by
  iintro ⟨Hpt, HΨ⟩
  iapply wpt_kill a loc ann kind pv ty bs ρ hk hstatic
  isplitl [Hpt]
  · iexact Hpt
  · iintro -
    iexact HΨ
...
theorem wpt_free {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(regionOwn (GF := GF) id a n (.own 1) bs ∗
      ...
      wpt M p Ls Θ k Ψ (killExpr an loc ann kind (cellPtr id a)) ρ := by
  iintro ⟨Hr, HΨ⟩
  iapply wpt_of_atomic (fun _ _ _ _ => free_atomic an loc ann kind id a n bs ρ hdyn) rfl rfl rfl hk
  isplitl [Hr]
  · iexact Hr
  · iintro %w ⟨%hw, Hd⟩
    subst hw
...
theorem wpt_free_emp {Ψ : SpikeVal → EnvStack → IProp GF}
    ...
    iprop(regionOwn (GF := GF) id a n (.own 1) bs ∗ Ψ (SpikeVal.pure Vunit) ρ) ⊢
      wpt M p Ls Θ k Ψ (killExpr an loc ann kind (cellPtr id a)) ρ := by
  iintro ⟨Hr, HΨ⟩
  iapply wpt_free an loc ann kind id a n bs ρ hk hdyn
  isplitl [Hr]
  · iexact Hr
  · iintro -
    iexact HΨ
...
theorem wpt_store_plain {Ψ : SpikeVal → EnvStack → IProp GF} (hΨ : AnnotInsensitive Ψ)
    ...
    iprop(pointsToCell M.tagDefs (GF := GF) pv (.own 1) ty bs ∗
      ...
      wpt M p Ls Θ k Ψ (storeExpr a loc ann ty pv cv mo) ρ := by
  iintro ⟨Hpt, HΨ⟩
  iapply wpt_store a loc ann ty pv cv mo mv bs ρ hk hmv hst
  isplitl [Hpt]
  · iexact Hpt
  iintro %fp Hpt'
  rw [hΨ]
...
theorem wpt_load_plain {Ψ : SpikeVal → EnvStack → IProp GF} (hΨ : AnnotInsensitive Ψ)
    ...
    iprop(pointsToCell M.tagDefs (GF := GF) pv dq ty bs ∗
      ...
      wpt M p Ls Θ k Ψ (loadExpr a loc ann ty pv mo) ρ := by
  iintro ⟨Hpt, HΨ⟩
  iapply wpt_load a loc ann ty pv mo dq bs ρ hk htrap
  isplitl [Hpt]
  · iexact Hpt
  iintro %fp Hpt'
  rw [hΨ]
    ...
    iprop(allocBudget (GF := GF) (planCost M.tagDefs (req :: rest)) ∗
      (∀ p : CerbMem.PointerValue,
        (pointsToCell M.tagDefs p (.own 1) req.ty
          ...
          ⌜0 < addrOf p ∧ addrOf p < 2 ^ 64⌝) -∗
        Ψ (SpikeVal.pure (Vobject (OVpointer p))) ρ)) ⊢
      wpt M p Ls Θ k Ψ (createExpr a loc ann (.IV aprov req.align) req.ty pref) ρ := by
  rw [planCost_cons]
  iintro ⟨Hb, HΨ⟩
...
theorem twp_ret {Φ : CoreRVal → IProp GF} (a1 b1 : List annot) (v : value) (ev0 : Fmap sym value)
  ...
  iintro H
  rw [(twp.unfold (e := (⟨ofValA (.pure a1 b1 v), ev0 :: evs,
    ⟨(p₀, ctx) :: κ, q, ℓ, lc, sp⟩, M⟩ : CoreRt))).to_eq]
  simp only [twp.pre, show ToVal.toVal (Val := CoreRVal) (⟨ofValA (.pure a1 b1 v),
    ev0 :: evs, ⟨(p₀, ctx) :: κ, q, ℓ, lc, sp⟩, M⟩ : CoreRt) = none from rfl]
...
/-- REMOVE-ANNOT under a frame at the raw TWP (`Step.ret_annot`,
    `Step.ret_annot_inv`). -/
theorem twp_ret_annot {Φ : CoreRVal → IProp GF} (a1 a2 b1 : List annot) (ds : List dyn_annotation)
    ...
    WP (⟨ofValA (.pure a2 b1 v), ρ, ⟨pc :: κ, q, ℓ, lc, sp⟩, M⟩ : CoreRt)
      ...
      WP (⟨ofValA (.annot a1 a2 b1 ds v), ρ, ⟨pc :: κ, q, ℓ, lc, sp⟩, M⟩ : CoreRt)
        @ Stuckness.NotStuck; ⊤ [{ Φ }] := by
  iintro H
  rw [(twp.unfold (e := (⟨ofValA (.annot a1 a2 b1 ds v), ρ,
    ⟨pc :: κ, q, ℓ, lc, sp⟩, M⟩ : CoreRt))).to_eq]
  simp only [twp.pre, show ToVal.toVal (Val := CoreRVal) (⟨ofValA (.annot a1 a2 b1 ds v), ρ,
    ⟨pc :: κ, q, ℓ, lc, sp⟩, M⟩ : CoreRt) = none from rfl]
  iintro %σ₁ %ns %obs %nt Hσ
  iapply fupd_mask_intro Std.LawfulSet.empty_subset
  iintro Hclose
  isplitr
...
/-- THE COLLAPSE INTO IRIS'S TOTAL WP, IN CPS OVER THE AMBIENT CONTROL
    (calls arc C3): under the total procedure specifications and the
    total block specifications of the current procedure, the total
    judgment at budget `k` entails the base Iris TWP of the configuration
    `⟨e, ρ, ⟨κ, p, ℓ, lc, sp⟩, M⟩` (E1: any current location and run
    supplies, ambient exactly as `ℓ`), given a continuation `K` for the
    delivered value at any final execution location, current location and
    supplies, with the value's own static annotation lists. Strong
    induction on the budget: the jump's `1 + m ≤ k` and the call's
    `1 + m + k' ≤ k` are the decreases. The return into the caller's
...
theorem wpt_sound_cps (p : Option sym) (Ls : LabelSpecT GF)
  ...
  | none =>
    ...
    | some q =>
      ...
      obtain rfl : (⟨κ, p, ℓ, lc, sp⟩ : Ctl).callPush (redexAnnots e) ctx f = rctl := hrctl.symm
      obtain rfl : σ₁ = σ₂ := hσ.symm
      imod Hclose with -
      imodintro
      isplit
    ...
    | none =>
      ...
      | succ k1 =>
        ...
        obtain ⟨e₂e, e₂ρ, ⟨rκ, rp, rℓ, rlc, rsp⟩, e₂M⟩ := e₂
        simp only at hlbl2 hs2
        obtain rfl : M = e₂M := hlbl2.symm
        have hce := Step.ctl_eq hs2 hcr htv
        obtain ⟨rfl, rfl, rfl⟩ : κ = rκ ∧ p = rp ∧ ℓ = rℓ := ⟨hce.1.symm, hce.2.1.symm, hce.2.2.symm⟩
        have hst : SameTail ρ e₂ρ := hs2.sameTail rfl
        imod Hstep $$ %(⟨e₂e, e₂ρ, ⟨κ, p, ℓ, rlc, rsp⟩, M⟩ : CoreRt) %σ₂ %([] : List CoreRt)
          %(⟨hs2, rfl, rfl⟩ :
            ((⟨e, ρ, ⟨κ, p, ℓ, lc, sp⟩, M⟩ : CoreRt), σ₁) -<([] : List Empty)>->
              ((⟨e₂e, e₂ρ, ⟨κ, p, ℓ, rlc, rsp⟩, M⟩ : CoreRt), σ₂, []))
          with ⟨HSI, Hwpt⟩
        imodintro
        isplit
        · ipureintro
...
theorem wpt_seq_sym_annot {Ψ : SpikeVal → EnvStack → IProp GF}
    (a pa : List annot) (x : sym) (bty : core_base_type)
    (e1 e2 : CoreExpr)
    (ev0 : Fmap sym value) (evs : List (Fmap sym value)) (k1 k2 : Nat) :
    wpt M p Ls Θ k1 (fun w ρ' => iprop(∃ (v : value) (ds : List dyn_annotation),
        ⌜w = SpikeVal.annot ds v⌝ ∗
        wpt M p Ls Θ k2 Ψ (Expr [] (Eannot ds e2)) (update_env (symPat pa x bty) v ρ')))
      e1 (ev0 :: evs) ⊢
      wpt M p Ls Θ (k1 + k2) Ψ (Expr a (Esseq (symPat pa x bty) e1 e2))
        (ev0 :: evs) := by
  induction k1 using Nat.strongRecOn generalizing e1 ev0 evs with
