...
def variants : List Variant := [
  ...
  { ctor := `CerberusHeapLang.Frag.memop_vals,
    shape := "`PtrEq` at two POINTER values with a STATE-INDEPENDENT verdict: null/null, null/concrete, function/function, same-provenance concrete pair",
  ...
  { ctor := `CerberusHeapLang.Frag.memop_vals,
    shape := "`PtrEq` at a FUNCTION pointer against a CONCRETE pointer (the verdict reads the state's `funptrmap`)",
