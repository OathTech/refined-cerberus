...
lemma suite rather than HeapLang's full library (allocation is a
LOCAL RULE ONLY: `wps_create` exists, but its allocator-cursor
resource is granted by NO adequacy launcher, so no heap-allocating
whole program is proved through it — 2026-09-01 skeptical re-audit
R-01; alloc arc P1). What it has instead is an object language
...
| `exhibitA_prod` (ProdExhibit.lean) | The production run of a self-contained create/store/load program IS the singleton Active execution delivering 7 with the exact final bytes. MIXED logical/operational proof (re-audit R-02): the create prefix and the six-round termination trace are handwritten operational rounds — NOT a create-rule consumer; only the store/load suffix crosses the logic (`SemTriple`) | file-system state and argv only — everything else discharged | production | trio + `runEffectful` |
...
| `counter_loop_certified_production` (ProdLoopExhibit.lean) | THE COUNTER LOOP ON THE SHIPPED PIPELINE (Phase 5): a SELF-CONTAINED program — the cell created by the engine's real allocator on the cold start (deterministically `pxPtr`), then the authored counter loop against it; conclusion: one Active execution, value `Vunit`, the created cell's final bytes pinned data-dependently (`CellCoh` of the final layout state). MIXED logical/operational proof (re-audit R-02): the cold-start create prefix is an explicit certified operational round chain (`Step.create` + `driverDone_step`) — NOT a create-rule consumer; the total judgment drives the loop suffix only | `0 ≤ n` + the engine's own fuel budget (`5·n + 7 ≤ 10^6`) + fs/argv | production | trio + `runEffectful` |
| `list_reverse_certified_production` (ProdLoopExhibit.lean) | THE FLAGSHIP'S PRODUCTION INSTANCE (Phase 5): a SELF-CONTAINED two-node build-and-reverse program — the chain engine-built cold (two creates + four field stores), then reversed by the authored flagship loop; conclusion: one Active execution whose delivered pointer heads a footprint seeded as the REVERSED chain (same engine-allocated ids, own values), satisfied by the final production memory. MIXED logical/operational proof (re-audit R-02): the chain build (two creates + four field stores) is an explicit certified operational round chain — NOT a create-rule consumer; the total judgment drives the reversal loop only | fs/argv only — everything else discharged (all data concrete) | production | trio + `runEffectful` |
...
suffixes only. No heap-allocating whole program is currently proved
through `wps_create` or a total create rule (R-01/R-02; the create
rule itself is LOCAL ONLY — its cursor resource is granted by no
adequacy launcher; alloc arc P1/P2).
...
is mixed logical/operational (R-02, as in the exhibit table): the
create prefix and the termination trace are handwritten operational
rounds; the logic carries the store/load suffix.
...
| `wps_create` is a LOCAL RULE ONLY — allocator resource not launchable: every adequacy launcher initializes the cursor ghost heap EMPTY, no launcher grants `cursorOwn`, so the create rule is unreachable from any engine-facing theorem (its one client, `struct_create_store_wps`, assumes the resource and ends at `wps`); no `wpt_create` exists | Alloc arc P1 (allocator-health launch invariant + cursor-granting launcher + public existential-pointer rule + total analog); closure tests in the R-01..R-11 table (`docs/2026-09-01_alloc-arc-plan.md`) | `Wps.lean` §CreateRule header; 2026-09-01 skeptical re-audit R-01 |
| The allocating production exhibits' create prefixes are proved OPERATIONALLY (handwritten certified driver rounds — `Step.create`/`driverDone_step` chains), not through the create logic rule: `exhibitA_prod`, `counter_loop_certified_production`, `list_reverse_certified_production` are MIXED logical/operational proofs | Alloc arc P2 (whole-program `wps`/`wpt` proofs, operational prefixes deleted from positive exhibits) | `ProdExhibit.lean`/`ProdLoopExhibit.lean` headers; 2026-09-01 skeptical re-audit R-02 |
...
| `Wps.lean` | The statement-stratified WP (the classical label-context judgment as a package-local guarded fixpoint): value/jump/step clauses, the jump-aware sequencing rules, the branch/entry rules, the small axioms at the stratum, THE GENERIC TYPED-SUBRANGE RULES (`wps_load_at`/`wps_store_at` over views + the derived whole-cell interior forms — Phase 2, F-04 retired), THE ALLOCATION RULE (`wps_create` through the allocator-cursor resource — LOCAL RULE ONLY: no adequacy launcher grants the resource, re-audit R-01), the per-label invariant loop rule `blockSpecs_intro` (partial correctness; the retired `blockSpecs_intro_variant` is replaced by the TOTAL stratum's `blockSpecsT`, Wpt.lean), and the Löb-tied collapse into the base WP | `wps`, `wps_seq`, `wps_seq_spec`, `wps_load_at`, `wps_store_at`, `wps_create`, `blockSpecs_intro`, `wps_sound` |
| `Wpt.lean` | THE TOTAL STATEMENT JUDGMENT (foundations Phase 3, audit F-02): `wpt M Ls k Ψ e ρ` by structural recursion on the step budget — variant-indexed label preconditions (`LabelSpecT`), MANDATORY back-edge decrease in the jump clause, the total rule set (sequencing/branch/entry/memory), `blockSpecsT` (replacing the retired variant lemma), and the collapse into the pinned Iris TotalWeakestPre | `wpt`, `wpt_run`, `wpt_seq`, `blockSpecsT`, `wpt_sound` |
...
| `TotalAdequacy.lean` | The two halves of total correctness, separated per the audit: TERMINATION over the unified relation (Iris `twp_total` consumed as-is) and THE GENERIC MEASURE→DRIVE-FUEL SIMULATION (the judgment's budget IS driveJ fuel — unconditional `.done` equations), on the step-monotone size potential `pot` (static fuel honesty, no run-length coupling) and the state-inert cone (final-state pins for action-free programs) | `wpt_strongly_normalizing`, `wpt_engine_boundJ`, `Frag.pot_step_bound` |
...
| `StructExhibit.lean` | THE FRESH-CLIENT TEST (Phase 2 acceptance): a two-field struct update (layout `{int x @ 0; int y @ 8}` in one 16-byte allocation — a third distinct layout) verified end-to-end with ZERO core-logic edits — every rule a one-line client instance of the generic subrange rules; plus the allocation CLIENT (create a fresh struct through the cursor resource and initialize a field — a LOCAL wps-level entailment whose premise assumes `cursorOwn`, not an adequacy consumer: re-audit R-01) | `struct_update_certified`, `struct_create_store_wps` |
...
| `ProdLoopExhibit.lean` | Phase 5: THE PRODUCTION LOOP EQUATIONS — loop programs certified as `CerbND.runND (Driver.drive …) (initial_driver_state …)` equations from the cold start (no package drive/driveJ in any statement); the counter and reversal programs are SELF-CONTAINED (cells/chains engine-built cold — MIXED proofs: the create prefixes are handwritten operational rounds, R-02) | `prod_run_eqJ`, `fib_certified_production`, `counter_loop_certified_production`, `list_reverse_certified_production` |
| `ProdExhibit.lean` | The demonstration: a self-contained program (create/store/load) run through the production pipeline delivers 7 with the exact final bytes (MIXED proof: create prefix + termination trace operational, R-02) | `exhibitA_prod` |
