...
alloc arc P1; the struct and create/store/load exhibits consume it
whole-program since alloc arc P2 slices 1-2; the two allocating LOOP
exports are still MIXED logical/operational pending P2 steps 4-5 —
R-02). What
...
| The allocating LOOP production exhibits' create prefixes are proved OPERATIONALLY (handwritten certified driver rounds — `Step.create`/`driverDone_step` chains), not through the create logic rule: `counter_loop_certified_production` and `list_reverse_certified_production` are MIXED logical/operational proofs (`exhibitA_prod` and the struct client CONVERTED at P2 slices 1-2; the Exhibit.lean termination trace retired for the generic total route `exhibitA_total`) | Alloc arc P2 steps 4-5 (whole-program `wpt` proofs for the counter and reversal; operational prefixes deleted from positive exhibits) | `ProdLoopExhibit.lean` header; 2026-09-01 skeptical re-audit R-02 |
