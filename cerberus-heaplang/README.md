...
| `Wps.lean` | The statement-stratified WP (the classical label-context judgment as a package-local guarded fixpoint): value/jump/step clauses, the jump-aware sequencing rules, the branch/entry rules, the small axioms at the stratum, THE GENERIC TYPED-SUBRANGE RULES (`wps_load_at`/`wps_store_at` over views + the derived whole-cell interior forms — Phase 2, F-04 retired), THE ALLOCATION RULE (`wps_create` through the allocator-cursor resource — Phase 2, D26 retired), the per-label invariant loop rule `blockSpecs_intro` (partial correctness; the retired `blockSpecs_intro_variant` is replaced by the TOTAL stratum's `blockSpecsT`, Wpt.lean), and the Löb-tied collapse into the base WP | `wps`, `wps_seq`, `wps_seq_spec`, `wps_load_at`, `wps_store_at`, `wps_create`, `blockSpecs_intro`, `wps_sound` |
