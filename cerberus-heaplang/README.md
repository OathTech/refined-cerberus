...
**Deliberately not here.** `kill`/free (no dispose rule yet: the
liveness token exists — the metadata cell's `alive` flag, K1 — and the
kill rules that consume it are K2/K3); procedures (no call
...
| Allocation metadata at a fraction as the exclusivity anchor AND the liveness token: the cell carries `alive` (RefinedC's `al_alive` and `freeable` collapsed into one cell — Cerberus erases the record on kill, so persistent-past-death metadata has no referent); no rule flips it yet | the kill rules (K2 static, K3 dynamic) are the ghost update to `alive := false` | `Heap.lean` header, "THE THREE ALLOCATION FACTS" |
