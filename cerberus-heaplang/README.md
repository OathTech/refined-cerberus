...
| The elaborator's Core is admitted as far as E1–E4 reach (annotations with the live location, `bound`, `Ivalignof`; the loaded-value currency, the `Unspecified` store, pure `case`, the tuple and weak binders; the C `int` arithmetic and the std.core calls through the file object; `unseq` with ccall-free components and the annotated-tuple weak binder) — t1's `main` is in `Frag` WHOLE and certified (`CorpusE0.t1Main_frag`, `t1_certified_production`; E4) — and NOT yet the rest of the corpus: negative actions (E5), `Eccall` (E6), a `run`/call or a `ccall`/`case`/`let`/`nd` component inside an `unseq` (no rule; NO-RULE/outside `Frag`); the binder rules at the plain-symbol binder are stated at BARE heads (the annotated head at the TUPLE binder is ruled since E4, `wps_wseq_tuple_annot`; the others mirrored, NO-RULE) | the dialect arc's remaining slices E5–E7 (`../docs/2026-09-04_emitted-core-dialect-design.md`) | "Scope, exactly"; `Soundness.lean` `Frag` header; `docs/2026-09-04_e1-notes.md`, `docs/2026-09-05_e2-notes.md`, `docs/2026-09-05_e3-notes.md`, `docs/2026-09-05_e4-notes.md` |
