...
alloc arc P1; the HEADLINE allocating exhibits are still MIXED
logical/operational proofs pending their P2 rewrite — R-02). What
...
equation at fuel 2 through the public total rule). The HEADLINE
allocating exhibits above do NOT yet consume that path — their
whole-program rewrites are alloc arc P2.
...
| Allocation rules + launch REPAIRED (alloc arc P1), consumers PARTIAL: the public `wps_create`/`wpt_create` (existential pointer, `allocCap` capacity, cursor-free statements) are launchable via the allocation-aware launchers (`launchResources` under `LaunchCoh`), with local consumers and the engine-facing smoke `alloc_create_launch_smoke` (AllocExhibit); BUT `struct_create_store_wps` still consumes the INTERNAL exact-cursor rule, and the headline allocating exhibits bypass the path entirely (R-02 row below) | Alloc arc P2 (whole-program `wps`/`wpt` rewrites of the struct/create-store-load/counter/reversal exports; operational prefixes deleted); R-01's closure test — deleting the public rule or the launch init must break a HEADLINE self-contained allocation theorem — is PENDING P2's consumers (status: "rules+launch landed" in the closure table, `docs/2026-09-01_alloc-arc-plan.md`) | `Wps.lean`/`Wpt.lean` §CreateRule headers; `docs/2026-09-01_p1-notes.md`; 2026-09-01 skeptical re-audit R-01 |
...
| `StructExhibit.lean` | THE FRESH-CLIENT TEST (Phase 2 acceptance): a two-field struct update (layout `{int x @ 0; int y @ 8}` in one 16-byte allocation — a third distinct layout) verified end-to-end with ZERO core-logic edits — every rule a one-line client instance of the generic subrange rules; plus the allocation CLIENT (create a fresh struct and initialize a field — a wps-level entailment still consuming the INTERNAL exact-cursor rule; its conversion to the public `allocCap` rule + an adequacy theorem is alloc arc P2 item 1) | `struct_update_certified`, `struct_create_store_wps` |
